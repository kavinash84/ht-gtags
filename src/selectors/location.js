...
export const getCurrentCity = createSelector([items], components => {
  let city = '';
...
export const getCurrentLocation = createSelector([location], currentPosition => currentPosition);
export const getStores = stores => (stores.items && stores.items.text ? stores.items.text : []);
export const getDestination = createSelector([getStores], stores => {
  const latlng = [];
  stores.forEach(item => {
    if (item.position) {
      latlng.push({
