...
@connect(
  ({ paymentstatus: { data, loaded, error }, userLogin: { isLoggedIn } }) => ({
    data,
    loaded,
    error,
    isLoggedIn
...
export default class PaymentSuccessContainer extends Component {
  ...
  componentDidMount() {
    ...
    const { error, data, history, isLoggedIn } = this.props;
    ...
    if (data) {
      ...
      if (
        error_message &&
        error_message.indexOf("Order Success details not found") >= 0
      ) {
        if (isLoggedIn) {
          return history.push("/my-orders");
        }
